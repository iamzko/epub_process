...
const QStringList IMAGE_EXTENSIONS = QStringList() << "jpg"
                                                   << "jpeg"
                                                   << "png"
                                                   << "gif"
                                                   << "tif"
...
const QStringList JPG_EXTENSIONS = QStringList() << "jpg"
                                                 << "jpeg";
const QStringList TIFF_EXTENSIONS = QStringList() << "tif"
                                                  << "tiff";
const QStringList MISC_TEXT_EXTENSIONS = QStringList() << "txt"
                                                       << "js";
const QStringList MISC_XML_EXTENSIONS = QStringList() << "smil"
                                                      << "xpgt"
                                                      << "pls";
const QStringList FONT_EXTENSIONS = QStringList() << "ttf"
                                                  << "ttc"
                                                  << "otf"
                                                  << "woff"
                                                  << "woff2";
const QStringList TEXT_EXTENSIONS = QStringList() << "xhtml"
                                                  << "html"
                                                  << "htm";
const QStringList STYLE_EXTENSIONS = QStringList() << "css";
const QStringList AUDIO_EXTENSIONS = QStringList() << "aac"
                                                   << "m4a"
                                                   << "mp3"
                                                   << "mpeg"
                                                   << "mpg"
                                                   ...
                                                   << "ogg"
                                                   << "opus";
const QStringList VIDEO_EXTENSIONS = QStringList() << "m4v"
                                                   << "mp4"
                                                   << "mov"
                                                   << "ogv"
                                                   << "webm"
                                                   << "vtt"
                                                   << "ttml";
const QStringList IMAGE_MIMEYPES = QStringList() << "image/gif"
                                                 << "image/jpeg"
                                                 << "image/png"
                                                 << "image/webp"
                                                 << "image/tiff";
...
const QStringList FONT_MIMETYPES = QStringList() << "application/x-font-ttf"
                                                 << "application/x-font-opentype"
                                                 << "application/vnd.ms-opentype"
                                                 << "application/font-woff"
                                                 << "application/font-sfnt"
                                                 << "font/woff2";
const QStringList AUDIO_MIMETYPES = QStringList() << "audio/mpeg"
                                                  << "audio/mp3"
                                                  << "audio/ogg"
                                                  << "audio/mp4"
                                                  << "audio/opus";
const QStringList VIDEO_MIMETYPES = QStringList() << "video/mp4"
                                                  << "video/ogg"
                                                  << "video/webm"
                                                  << "text/vtt"
...
QString MediaTypes::GetGroupFromMediaType(const QString& media_type, const QString& fallback)
{
    ...
    if (group.isEmpty()) {
        if (media_type.startsWith("image/"))
            group = "Images";
        if (media_type.startsWith("application/font"))
            group = "Fonts";
        if (media_type.startsWith("application/x-font"))
            group = "Fonts";
        ...
        if (media_type.startsWith("video/"))
            group = "Video";
        if (media_type.contains("adobe") && media_type.contains("template"))
            group = "Misc";
    }
    if (group.isEmpty())
        return fallback;
